function CloseSVG() {
  return (
    <svg className='hover:stroke-[#c5303e] w-5 h-5 lg:w-6 lg:h-6 stroke-error hover:cursor-pointer' viewBox="-0.5 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M3 21.32L21 3.32001"  strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M3 3.32001L21 21.32"  strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  )
}

export default CloseSVG